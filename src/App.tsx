...
/* ============ 작은 유틸 ============ */
const S = (v: any) => String(v ?? "");            // 어떤 값이 와도 문자열
...
/** JSON 전체를 안전하게 정규화 (객체/숫자/null → 문자열) */
function normalizeConfig(json: any): Config {
  ...
  for (const k of Object.keys(rawCats)) {
    cats[S(k)] = normalizeWords(rawCats[k]);
  }
    ...
    if (this.state.err) {
      return (
        <div style={{ padding: 16, color: "#b91c1c" }}>
          ...
          <pre style={{ whiteSpace: "pre-wrap" }}>{S(this.state.err?.message || this.state.err)}</pre>
        </div>
      );
    }
      ...
      {!cat ? (
            ...
            {ord.map((raw) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#64748b" }}>단어 {count}개</div>
                </button>
              );
            })}
          </div>
        ...
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <h2 style={{ fontWeight: 700 }}>{S(cat)} BINGO</h2>
